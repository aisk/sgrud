...
`--no-stacks`, `--no-tasks`, `--no-gc`, `--no-children`, `--no-ipc`를 사용하면 필요
없는 섹션을 인터페이스나 덤프에서 제외할 수 있습니다.

`examples/demo_app.py`는 모든 탭에 볼거리를 채워 주는 대상 프로그램입니다. 바쁜 스레드와 블록된 스레드, asyncio 태스크 트리, `multiprocessing` 워커, 파이프, 소켓, 공유 메모리, 자식 프로세스가 쥐고 있는 파일 잠금을 돌립니다. 실행한 뒤 출력되는 pid에 sgrud를 붙이면 됩니다.
